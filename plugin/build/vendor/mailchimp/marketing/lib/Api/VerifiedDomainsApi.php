...
class VerifiedDomainsApi
{
    ...
    protected function deleteDomainRequest($domain_name)
    {
        ...
        if ($domain_name === null || \is_array($domain_name) && \count($domain_name) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $domain_name when calling ');
        }
        ...
        // path params
        if ($domain_name !== null) {
            $resourcePath = \str_replace('{' . 'domain_name' . '}', \Lauant\MCD\MailchimpMarketing\ObjectSerializer::toPathValue($domain_name), $resourcePath);
        }
        // body params
        $_tempBody = null;
        if ($multipart) {
