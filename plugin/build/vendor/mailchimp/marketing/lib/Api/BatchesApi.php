...
class BatchesApi
{
    ...
    protected function deleteRequestRequest($batch_id)
    {
        ...
        if ($batch_id === null || \is_array($batch_id) && \count($batch_id) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $batch_id when calling ');
        }
        ...
        // path params
        if ($batch_id !== null) {
            $resourcePath = \str_replace('{' . 'batch_id' . '}', \Lauant\MCD\MailchimpMarketing\ObjectSerializer::toPathValue($batch_id), $resourcePath);
        }
        // body params
        $_tempBody = null;
        if ($multipart) {
