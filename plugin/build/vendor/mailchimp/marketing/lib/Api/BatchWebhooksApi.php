...
class BatchWebhooksApi
{
    ...
    protected function removeRequest($batch_webhook_id)
    {
        ...
        if ($batch_webhook_id === null || \is_array($batch_webhook_id) && \count($batch_webhook_id) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $batch_webhook_id when calling ');
        }
        ...
        if ($batch_webhook_id !== null) {
            $resourcePath = \str_replace('{' . 'batch_webhook_id' . '}', \Lauant\MCD\MailchimpMarketing\ObjectSerializer::toPathValue($batch_webhook_id), $resourcePath);
        }
        // body params
        $_tempBody = null;
        if ($multipart) {
