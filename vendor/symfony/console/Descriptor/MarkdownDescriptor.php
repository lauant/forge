 ...
 * @internal
 */
class MarkdownDescriptor extends \Lauant\Forge\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function describeCommand(\Lauant\Forge\Symfony\Component\Console\Command\Command $command, array $options = array())
    {
        ...
        $this->write($command->getName() . "\n" . \str_repeat('-', \Lauant\Forge\Symfony\Component\Console\Helper\Helper::strlen($command->getName())) . "\n\n" . '* Description: ' . ($command->getDescription() ?: '<none>') . "\n" . '* Usage:' . "\n\n" . \array_reduce(\array_merge(array($command->getSynopsis()), $command->getAliases(), $command->getUsages()), function ($carry, $usage) {
            return $carry . '  * `' . $usage . '`' . "\n";
        }));
    ...
    protected function describeApplication(\Lauant\Forge\Symfony\Component\Console\Application $application, array $options = array())
    {
        ...
        $this->write($application->getName() . "\n" . \str_repeat('=', \Lauant\Forge\Symfony\Component\Console\Helper\Helper::strlen($application->getName())));
        foreach ($description->getNamespaces() as $namespace) {
            if (\Lauant\Forge\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
