 ...
 */
class ProgressHelper extends \Lauant\Forge\Symfony\Component\Console\Helper\Helper
{
    ...
    public function __construct($triggerDeprecationError = \true)
    {
        if ($triggerDeprecationError) {
            @\trigger_error('The ' . __CLASS__ . ' class is deprecated since Symfony 2.5 and will be removed in 3.0. Use the Symfony\\Component\\Console\\Helper\\ProgressBar class instead.', \E_USER_DEPRECATED);
        }
    }
    /**
     ...
     */
    public function start(\Lauant\Forge\Symfony\Component\Console\Output\OutputInterface $output, $max = null)
    {
        ...
        $this->output = $output->isDecorated() ? $output : new \Lauant\Forge\Symfony\Component\Console\Output\NullOutput();
        $this->lastMessagesLength = 0;
        $this->barCharOriginal = '';
        if (null === $this->format) {
            switch ($output->getVerbosity()) {
                case \Lauant\Forge\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET:
    ...
    public function setCurrent($current, $redraw = \false)
    {
        if (null === $this->startTime) {
            throw new \Lauant\Forge\Symfony\Component\Console\Exception\LogicException('You must start the progress bar before calling setCurrent().');
        }
        $current = (int) $current;
    ...
    {
        if (null === $this->startTime) {
            throw new \Lauant\Forge\Symfony\Component\Console\Exception\LogicException('You must start the progress bar before calling display().');
        }
        $message = $this->format;
    ...
    {
        if (null === $this->startTime) {
            throw new \Lauant\Forge\Symfony\Component\Console\Exception\LogicException('You must start the progress bar before calling finish().');
        }
        if (null !== $this->startTime) {
            if (!$this->max) {
     ...
     */
    private function overwrite(\Lauant\Forge\Symfony\Component\Console\Output\OutputInterface $output, $message)
    {
